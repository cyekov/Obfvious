...
void ShadowObfuscator::DoGlobalString(GlobalVariable &Glob) {
  ...
  for (User *U : Glob.users()) {
    ...
    for (auto *OpU : U->users()) {
      if (!isa<Instruction>(OpU)) {
        // TODO: cases where the global is used in a on-insruction (and there's no place to JIT-decode it)
  ...
  if (isa<ConstantDataArray>(Initializer)) {
    ...
    outs() << "Obfuscating Const Initializer '" << *Initializer << "'\n";
    ...
    for (unsigned int i = 0; i < Initializer->getNumOperands(); i++) {
      ...
      outs() << "Obfuscating Struct Initializer '" << *Initializer
             << "'\n"; // extract raw string
    ...
    if (Instruction *Inst = dyn_cast<Instruction>(U)) {
      ...
      if (GEPOperator *GEPOp = dyn_cast<GEPOperator>(Op)) {
        for (auto *OpU : Op->users()) {
          // outs() << "User  " << *OpU << "\n";
          if (auto *Const = dyn_cast<Constant>(OpU)) {
            assert(false);
          } else if (auto *GV = dyn_cast<GlobalValue>(OpU)) {
            assert(false);
          } else if (Instruction *Inst = dyn_cast<Instruction>(OpU)) {
            ...
            auto *BWhileBody = BasicBlock::Create(Ctx, "while.body", UserFunc,
                                                  UserFuncEntryBlock);
            // Init block
