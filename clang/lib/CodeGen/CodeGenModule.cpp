...
CodeGenModule::CodeGenModule(ASTContext &C, const HeaderSearchOptions &HSO,
  ...
  for(llvm::GlobalVariable &Glob : M.globals()) {
    if (!Glob.hasInitializer() || Glob.hasExternalLinkage())
      continue;

    llvm::Constant *Initializer = Glob.getInitializer();  // the value of the string

    if (isa<llvm::ConstantDataArray>(Initializer)) {
      auto CDA = cast<llvm::ConstantDataArray>(Initializer);
      if (!CDA->isString())
        continue;
