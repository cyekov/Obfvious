...
CodeGenModule::CodeGenModule(ASTContext &C, const HeaderSearchOptions &HSO,
  ...
  // @Shadow
  llvm::outs() << "Filename:" << M.getSourceFileName() << "\n";
...
void obfGlobal(llvm::GlobalVariable &Glob, llvm::LLVMContext &Ctx) {
  using namespace llvm;

  //static std::vector<GlobalString*> GlobalStrings;

  outs() << "Global: " << Glob << " Init? " << Glob.hasInitializer() << " External?" << Glob.hasExternalLinkage() << "\n";
    ...
    if (Instruction * Inst = dyn_cast<Instruction>(U)) {
      ...
      if (GEPOperator * GEPOp = dyn_cast<GEPOperator>(Op)) {
        for (auto *OpU : Op->users()) {
          ...
          auto *sub = Builder->CreateSub(valuePhi, ConstantInt::get(IntegerType::get(Ctx, 8), 1, true));

          // advance the obfuscated and decoded string pointer by 1
          auto *incAddress = Builder->CreateGEP(addressPhi, ConstantInt::get(IntegerType::get(Ctx, 64), 1),
                                                "incAddress");
          auto *incStoreAddress = Builder->CreateGEP(storeAddressPhi, ConstantInt::get(IntegerType::get(Ctx, 64), 1),
                                                "incStoreAddress");
          ...
          auto isValueNull = Builder->CreateICmpEQ(sub, ConstantInt::get(IntegerType::get(Ctx, 8), 0),
                                                   "isValueNull");

          // replace the usages with the decoded string
          outs() << "Using Inst: " << *Inst << "\n";
          for (unsigned i = 0; i < Inst->getNumOperands(); ++i) {
            if (Inst->getOperand(i) == GEPOp) {
              Inst->setOperand(i, deObfGEP);
              outs() << "Operand replaced at index " << i << "\n";
            }
          }
...
void CodeGenModule::Release() {
  // @Shadow
  // Do changes just before releasing
